...
int main()
{
    ...
    ArrayList* listaEmpleadosFiltrados;
    ...
    if(parser_parseEmpleados("data.csv",listaEmpleados)==1)
    {
        ...
        listaEmpleadosFiltrados = al_filter(listaEmpleados,em_trabajaMasDe120Horas);

        // Generar archivo de salida
        if(generarArchivoFiltrado("sueldos.csv",listaEmpleadosFiltrados)==1)
        {
...
int generarArchivoFiltrado(char* fileName,ArrayList* listaEmpleados)
{
    return parser_guardarEmpleado(fileName, listaEmpleados);
}
